...
│   ├── annotated/                 # 标注数据 (生成)
│   ├── profiles/                  # 角色档案 (生成)
│   ├── airp2.sqlite3              # SQLite（用户/会话/小说/job 元数据）
│   ├── users/                     # 多用户私有空间（强隔离）
│   │   └── <user_id>/
│   │       ├── novels/<novel_id>/...     # 每本小说独立 input/chapters/scenes/...
│   │       └── sessions/(global|novels)/ # 用户会话存储（按 actor 隔离）
│   └── guests/                    # 游客空间（仅会话）
│       └── <guest_id>/sessions/...
│
├── 📁 vector_db/                   # 向量数据库 (生成)
│   └── users/<user_id>/<novel_id>/...    # 多用户/多小说隔离向量库目录（Qdrant local）
└── 📁 logs/                        # 运行日志 (生成)
    └── users/<user_id>/novels/<novel_id>/job_*.log # 每用户每小说 job 日志
```
